...
orders_list = [0] * (max_event_time + 1)
...
for key, item in orders_dict.items():
    ...
    prev_sum = orders_dict[key]['cur_cost']
    sum_cost += orders_dict[key]['cur_cost']
    orders_dict[key]['sum_cost'] = sum_cost
    orders_dict[key]['prev_time'] = prev_time
    prev_time = orders_dict[key]['cur_time']
    sum_time += orders_dict[key]['cur_time']
...
for j in range(Q):
    ...
    while i < len(orders_list_slice):
        if orders_list_slice[i] and not s_min:
            search_min = orders_list_slice[i]
            s_min = True
        if orders_list_slice[len(orders_list_slice) - 1 - i] and not s_max:
    ...
    if ask_type == 1 and search_min and search_max:
        summary = orders_dict[search_max]['sum_cost'] - orders_dict[search_min]['sum_cost'] + orders_dict[search_min]['cur_cost']
    elif ask_type == 2 and search_min and search_max:
        summary = orders_dict[search_max]['sum_time'] - orders_dict[search_min]['sum_time'] + orders_dict[search_min]['cur_time']
